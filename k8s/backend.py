...
from apidefs.backend import Backend
from k8s.pod import Pod
from k8s.service import Service
from k8s.deployment import Deployment
from k8s.utils import k8s_ports_to_metadata_ports
from apidefs.metadata import ImageMetadata
import k8s.client as k8s_client
...
class K8sBackend(Backend):
    ...
    def __init__(self, api_key=None, logging_level=logging.INFO, logging_kwargs=None, cleanup=None):
        ...
        if K8sCleanupPolicy.NOTHING in self.cleanup and len(self.cleanup) != 1:
            raise KubeException("Cleanup policy NOTHING cannot be combined with other values")
