...
from k8s.client import get_core_api
...
class Pod(object):
    ...
    def __init__(self, namespace, name=None, spec=None, from_template=None):
        ...
        if (from_template is not None) and (name is not None or spec is not None):
            raise KubeException('from_template cannot be passed to constructor at the same time'
                                ' with name or spec')
            ...
            raise KubeException('to create pod you need to specify pod template or'
    ...
    def delete(self):
        ...
        if status.status == 'Failure':
            raise KubeException("Deletion of Pod failed")
