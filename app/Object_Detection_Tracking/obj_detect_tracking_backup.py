...
def check_args(args):
  ...
  # Temporarily use args.state_store to specify dapr state store 
  assert args.state_store is not None 
...
if __name__ == "__main__":
  ...
  with tf.Session(config=tfconfig) as sess:
    ...
    while True:
      ...
      #   cur_frame += 1
      #   continue
      print("time stamp:", vcap.get(cv2.CAP_PROP_POS_MSEC))
      if not suc:
          break
      ...
      # print(resized_image.shape, resized_image.dtype)
      feed_dict = model.get_feed_dict_forward(resized_image)
      time0 = time.time()
      # print("decoder:", time0-start_time)
      time1_1 = time.time()
      if args.get_box_feat:
        ...
        t1 = time.time()
        # write_state(d, args.state_store, args.app_name, {"boxes": final_boxes, "labels": final_labels, "probs": final_probs, "tracker": tracker_dict})
        t2 = time.time()
        # print("write state time cost:", t2-t1)
        time1 = time.time()
        # final_boxes, final_labels, final_probs, tracker_dict = read_state(d, args.state_store, args.app_name)
        time2 = time.time()
        # print("read state time cost:", time2-time1)
        for tracking_obj in tracking_objs:
          target_tracking_obs = [tracking_obj]
          print(tracking_obj, targetid2class)
    ...
    # if args.test_frame_extraction:
      # tqdm.write("video %s got %s frames, opencv said frame count is %s" % (
