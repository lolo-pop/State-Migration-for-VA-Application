...
class PerformanceLogger(object):
  ...
  def __init__(self, gpu_ids, interval, pid1, pid2, pid3):
    ...
    self.performance_check_thread = threading.Thread(
        target=self.log_util_fn, args=(pid1, pid2, pid3,))
    self.performance_check_thread.daemon = True

  def log_util_fn(self, pid1, pid2, pid3):
    while True:
      ...
      cpu_percent1 = p1.cpu_percent(interval=0.5) # better set interval second to calculate like:  p.cpu_percent(interval=0.5)
      mem_percent1 = p1.memory_percent()
      cpu_percent2 = p2.cpu_percent(interval=0.5) # better set interval second to calculate like:  p.cpu_percent(interval=0.5)
      mem_percent2 = p2.memory_percent()
      cpu_percent3 = p3.cpu_percent(interval=0.5) # better set interval second to calculate like:  p.cpu_percent(interval=0.5)
      mem_percent3 = p3.memory_percent()
      ...
      self.logs["cpu_utilization1"].append(cpu_percent1)
      self.logs["cpu_utilization2"].append(cpu_percent2)
      self.logs["cpu_utilization3"].append(cpu_percent3)
      self.logs["mem_utilization1"].append(mem_percent1)
      self.logs["mem_utilization2"].append(mem_percent2)
      self.logs["mem_utilization3"].append(mem_percent3)
      self.logs["ram_used"].append(mem_percent1+mem_percent2+mem_percent3)
