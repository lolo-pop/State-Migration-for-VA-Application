...
def detector(frame, cur_frame):
  ...
  print("seesion test", flush=True)
  print(args.is_load_from_pb, flush=True)
  ...
  if args.get_box_feat:
    ...
    featfile = os.path.join(feat_out_path, "{}.npy".format(cur_frame))
...
def subscribe():
  subscriptions = [{'pubsubname': 'pubsub', 'topic': 'pubsub', 'route': 'pubsub'}]
  print("subscribe start!", flush=True)
...
if __name__ == "__main__":
  ...
  global sess
  t1 = time.time()
  model, sess = lazy_cuda_init()
  print(type(model), type(sess))
  t2 = time.time()
